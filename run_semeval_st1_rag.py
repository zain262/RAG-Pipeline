...
"""
    ...
    run_df = (
    df[df["semeval-split"].astype(str).str.lower().isin(splits)]
    .copy()
    )
    ...
    val = out[out["semeval-split"].str.lower().eq("validation")].dropna(
    subset=[
    ...
    if len(val) > 0:
        h_acc = accuracy_score(val["hazard-category_true"], val["hazard-category_pred"])
        h_prec = precision_score(val["hazard-category_true"], val["hazard-category_pred"],
                                average="macro", zero_division=0)
        h_f1 = f1_score(val["hazard-category_true"], val["hazard-category_pred"],
                        average="macro", zero_division=0)

        p_acc = accuracy_score(val["product-category_true"], val["product-category_pred"])
        p_prec = precision_score(val["product-category_true"], val["product-category_pred"],
                                average="macro", zero_division=0)
        p_f1 = f1_score(val["product-category_true"], val["product-category_pred"],
                        average="macro", zero_division=0)
        p_rec = recall_score(
        val["product-category_true"],
        val["product-category_pred"],
        average="macro",
        zero_division=0
        )

        h_rec = recall_score(
        val["hazard-category_true"],
        val["hazard-category_pred"],
        average="macro",
        zero_division=0
        )
        print(
