...
def norm_for_f1(x: Any) -> str:
    """
    Normalize strings for an approximate ST2 F1.
    This is NOT the official SemEval ST2 metric, but a helpful sanity check:
    - lowercased
    - whitespace-collapsed
    - None/NaN mapped to a special token
    """
...
"""
    ...
    val = out_df[out_df["semeval-split"].astype(str).str.lower().eq("validation")].copy()
    if has_gold_hazard and has_gold_product and len(val) > 0:
        hz_true = val["hazard_true"].apply(norm_for_f1)
        hz_pred = val["hazard_pred"].apply(norm_for_f1)
        pr_true = val["product_true"].apply(norm_for_f1)
        pr_pred = val["product_pred"].apply(norm_for_f1)

        h_f1 = f1_score(hz_true, hz_pred, average="macro")
        p_f1 = f1_score(pr_true, pr_pred, average="macro")
        print(f"validation macro-F1 (approx) hazard={h_f1:.4f} product={p_f1:.4f}")
